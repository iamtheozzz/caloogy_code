...
async function fetchCandles(symbol, interval = '1H', limit = 100) {
    ...
    if (STOCK_SYMBOLS.has(symbol)) {
        candles = await fetchYahooCandles(symbol, interval, cap);
    } else {
        ...
        if (okxId) {
            try {
                ...
                if (j.data && j.data.length > 0) {
                    candles = j.data.slice().reverse().map(k => ({
    ...
    // Persist to local DB (non-blocking, failure is non-fatal)
    db.upsertCandles(symbol, interval, candles, 'api').catch(e =>
