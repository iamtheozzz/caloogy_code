...
'print(json.dumps({"plots": plots, "markers": markers}))',
    ...
    py_vwap_dev: _PY_H + [
'# [Python] VWAP Deviation (50-bar)',
'P = 50',
'vw = [None]*n',
'for i in range(P-1, n):',
'    tv = sum(vo[j] for j in range(i-P+1, i+1))',
'    vw[i] = sum(((hi[j]+lo[j]+cl[j])/3)*vo[j] for j in range(i-P+1, i+1)) / tv if tv else None',
'plo("VWAP", vw, "#10b981")',
'dev = 0.02',
'for i in range(1, n):',
'    if vw[i] is None or vw[i-1] is None: continue',
'    d  = (cl[i]   - vw[i])   / vw[i]',
'    pd = (cl[i-1] - vw[i-1]) / vw[i-1]',
'    if pd < -dev and d >= -dev: mrk(i, "buy")',
'    if pd <  dev and d >=  dev: mrk(i, "sell")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_triple_ema: _PY_H + [
'# [Python] Triple EMA (9/21/55)',
'e9  = ema(cl, 9)',
'e21 = ema(cl, 21)',
'e55 = ema(cl, 55)',
'plo("EMA 9",  e9,  "#f59e0b")',
'plo("EMA 21", e21, "#6366f1")',
'plo("EMA 55", e55, "#ef4444")',
'for i in range(1, n):',
'    if not e9[i] or not e21[i] or not e55[i]: continue',
'    bull = e21[i] > e55[i]',
'    bear = e21[i] < e55[i]',
'    if bull and e9[i-1] <= e21[i-1] and e9[i] > e21[i]: mrk(i, "buy")',
'    if bear and e9[i-1] >= e21[i-1] and e9[i] < e21[i]: mrk(i, "sell")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),
...
'    ml = [mf[i] - ms[i] if mf[i] and ms[i] else None for i in range(n)]',
...
'print(json.dumps({"plots": plots, "markers": markers}))',
    ...
    py_bb_squeeze: _PY_H + [
'# [Python] Bollinger Band Squeeze (Keltner)',
'P = 20',
'bb = bollinger(cl, P, 2.0)',
'at = atr(P)',
'em = ema(cl, P)',
'kU = [em[i] + 1.5*at[i] if em[i] and at[i] else None for i in range(n)]',
'kL = [em[i] - 1.5*at[i] if em[i] and at[i] else None for i in range(n)]',
'plo("BB Upper", [v[2] if v else None for v in bb], "#6366f1")',
'plo("BB Mid",   [v[1] if v else None for v in bb], "#94a3b8")',
'plo("BB Lower", [v[0] if v else None for v in bb], "#6366f1")',
'plo("KC Upper", kU, "#f59e0b")',
'plo("KC Lower", kL, "#f59e0b")',
'for i in range(1, n):',
'    if not bb[i] or not bb[i-1] or kU[i] is None: continue',
'    was = bb[i-1][2] < kU[i-1] and bb[i-1][0] > kL[i-1]',
'    now = bb[i][2] >= kU[i] or bb[i][0] <= kL[i]',
'    if was and now:',
'        if cl[i] > bb[i][1]: mrk(i, "buy",  "SQZ")',
'        else:                 mrk(i, "sell", "SQZ")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_adx_filter: _PY_H + [
'# [Python] ADX Trend Filter + EMA Cross',
'P = 14',
'tr = [hi[0]-lo[0]] + [max(hi[i]-lo[i], abs(hi[i]-cl[i-1]), abs(lo[i]-cl[i-1])) for i in range(1, n)]',
'pdm = [max(hi[i]-hi[i-1], 0) if hi[i]-hi[i-1] > lo[i-1]-lo[i] else 0 for i in range(1, n)]',
'mdm = [max(lo[i-1]-lo[i], 0) if lo[i-1]-lo[i] > hi[i]-hi[i-1] else 0 for i in range(1, n)]',
'pdm = [0] + pdm; mdm = [0] + mdm',
'def smth(a):',
'    r = [None]*n; r[P] = sum(a[1:P+1])',
'    for i in range(P+1, n): r[i] = r[i-1] - r[i-1]/P + a[i]',
'    return r',
'satr = smth(tr); spdm = smth(pdm); smdm = smth(mdm)',
'dip = [100*spdm[i]/satr[i] if satr[i] else None for i in range(n)]',
'dim = [100*smdm[i]/satr[i] if satr[i] else None for i in range(n)]',
'dx  = [100*abs(dip[i]-dim[i])/(dip[i]+dim[i]) if dip[i] and dim[i] and dip[i]+dim[i] else None for i in range(n)]',
'adxv= [None]*n',
'start = next((i for i in range(n) if dx[i] is not None), None)',
'if start is not None:',
'    adxv[start] = dx[start]',
'    for i in range(start+1, n):',
'        adxv[i] = (adxv[i-1]*(P-1) + dx[i]) / P if dx[i] is not None and adxv[i-1] is not None else adxv[i-1]',
'f9 = ema(cl, 9); f21 = ema(cl, 21)',
'plo("EMA 9",  f9,  "#f59e0b")',
'plo("EMA 21", f21, "#6366f1")',
'plo("DI+", dip, "#10b981")',
'plo("DI-", dim, "#ef4444")',
'for i in range(1, n):',
'    if f9[i] is None or f21[i] is None or adxv[i] is None: continue',
'    if adxv[i] < 25: continue',
'    if f9[i-1] <= f21[i-1] and f9[i] > f21[i] and dip[i] and dim[i] and dip[i] > dim[i]: mrk(i, "buy")',
'    if f9[i-1] >= f21[i-1] and f9[i] < f21[i] and dim[i] and dip[i] and dim[i] > dip[i]: mrk(i, "sell")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),
...
'print(json.dumps({"plots": plots, "markers": markers}))',
    ...
    py_ichimoku: _PY_H + [
'# [Python] Ichimoku Cloud',
'def midpt(a, i, p): return (max(a[i-p+1:i+1]) + min(a[i-p+1:i+1])) / 2',
'tk = [midpt(hi, i, 9)  if i >= 8  else None for i in range(n)]',
'kj = [midpt(hi, i, 26) if i >= 25 else None for i in range(n)]',
'sa = [(tk[i]+kj[i])/2  if tk[i] and kj[i] else None for i in range(n)]',
'sb = [midpt(hi, i, 52) if i >= 51 else None for i in range(n)]',
'plo("Tenkan (9)",  tk, "#f59e0b")',
'plo("Kijun (26)",  kj, "#ef4444")',
'plo("Senkou A",    sa, "#10b981")',
'plo("Senkou B",    sb, "#a855f7")',
'for i in range(1, n):',
'    if not tk[i] or not kj[i] or not sa[i] or not sb[i]: continue',
'    ct = max(sa[i], sb[i]); cb = min(sa[i], sb[i])',
'    abv = cl[i] > ct; blw = cl[i] < cb',
'    bul = tk[i-1] <= kj[i-1] and tk[i] > kj[i]',
'    ber = tk[i-1] >= kj[i-1] and tk[i] < kj[i]',
'    if abv and bul: mrk(i, "buy",  "TK↑")',
'    if blw and ber: mrk(i, "sell", "TK↓")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_donchian_turtle: _PY_H + [
'# [Python] Donchian Turtle Breakout',
'def dc(p): return [(max(cl[i-p+1:i+1]), min(cl[i-p+1:i+1])) if i>=p-1 else None for i in range(n)]',
'd20 = dc(20); d10 = dc(10)',
'plo("DC20 High", [v[0] if v else None for v in d20], "#10b981")',
'plo("DC20 Low",  [v[1] if v else None for v in d20], "#ef4444")',
'plo("DC10 High", [v[0] if v else None for v in d10], "#6366f1")',
'plo("DC10 Low",  [v[1] if v else None for v in d10], "#a855f7")',
'pos = 0',
'for i in range(1, n):',
'    if not d20[i] or not d20[i-1]: continue',
'    if pos == 0:',
'        if cl[i] > d20[i-1][0]: mrk(i, "buy",  "BK↑"); pos =  1',
'        elif cl[i] < d20[i-1][1]: mrk(i, "sell", "BK↓"); pos = -1',
'    elif pos == 1:',
'        if d10[i] and cl[i] < d10[i][1]: mrk(i, "sell", "Ex"); pos = 0',
'    elif pos == -1:',
'        if d10[i] and cl[i] > d10[i][0]: mrk(i, "buy",  "Ex"); pos = 0',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_obv_confirm: _PY_H + [
'# [Python] OBV Trend Confirmation',
'pf = ema(cl, 12); ps = ema(cl, 26)',
'plo("EMA 12", pf, "#f59e0b")',
'plo("EMA 26", ps, "#6366f1")',
'obv = [0]*n',
'for i in range(1, n):',
'    obv[i] = obv[i-1] + (vo[i] if cl[i] > cl[i-1] else -vo[i] if cl[i] < cl[i-1] else 0)',
'of = ema(obv, 12); os_ = ema(obv, 26)',
'for i in range(1, n):',
'    if of[i] is None or os_[i] is None or pf[i] is None or ps[i] is None: continue',
'    pbul = pf[i-1] <= ps[i-1] and pf[i] > ps[i]',
'    pber = pf[i-1] >= ps[i-1] and pf[i] < ps[i]',
'    ocbu = of[i-1] <= os_[i-1] and of[i] > os_[i]',
'    ocbe = of[i-1] >= os_[i-1] and of[i] < os_[i]',
'    if pbul and of[i] > os_[i]: mrk(i, "buy",  "V↑")',
'    if pber and of[i] < os_[i]: mrk(i, "sell", "V↓")',
'    if ocbu and pf[i] > ps[i]:  mrk(i, "buy",  "OBV")',
'    if ocbe and pf[i] < ps[i]:  mrk(i, "sell", "OBV")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_hull_momentum: _PY_H + [
'# [Python] Hull MA Momentum (9/25/50)',
'def hma(a, p):',
'    h = int(math.sqrt(p))',
'    w  = ema(a, p//2); f = ema(a, p)',
'    raw = [2*w[i] - f[i] if w[i] and f[i] else None for i in range(n)]',
'    return ema(raw, h)',
'h9  = hma(cl, 9)',
'h25 = hma(cl, 25)',
'h50 = hma(cl, 50)',
'plo("HMA 9",  h9,  "#f59e0b")',
'plo("HMA 25", h25, "#6366f1")',
'plo("HMA 50", h50, "#94a3b8")',
'for i in range(2, n):',
'    if not h9[i] or not h25[i] or not h50[i]: continue',
'    up   = h50[i] > h50[i-1]',
'    down = h50[i] < h50[i-1]',
'    x9u  = h9[i-1] <= h25[i-1] and h9[i] > h25[i]',
'    x9d  = h9[i-1] >= h25[i-1] and h9[i] < h25[i]',
'    if up   and x9u: mrk(i, "buy")',
'    if down and x9d: mrk(i, "sell")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_supertrend_rsi: _PY_H + [
'# [Python] Supertrend(10,3) + RSI filter',
'P = 10; mult = 3.0',
'at = atr(P)',
'rv = rsi(cl, 14)',
'plo("EMA 20", ema(cl, 20), "#94a3b8")',
'up = [None]*n; dn = [None]*n; dir_ = [0]*n',
'for i in range(P, n):',
'    if at[i] is None: continue',
'    bu = (hi[i]+lo[i])/2 - mult*at[i]',
'    bd = (hi[i]+lo[i])/2 + mult*at[i]',
'    up[i] = max(bu, up[i-1]) if up[i-1] and cl[i-1] > up[i-1] else bu',
'    dn[i] = min(bd, dn[i-1]) if dn[i-1] and cl[i-1] < dn[i-1] else bd',
'    if dir_[i-1] == -1 and cl[i] > dn[i]:  dir_[i] =  1',
'    elif dir_[i-1] == 1 and cl[i] < up[i]: dir_[i] = -1',
'    else: dir_[i] = dir_[i-1] if dir_[i-1] != 0 else (1 if cl[i] > (bu+bd)/2 else -1)',
'for i in range(1, n):',
'    if rv[i] is None or dir_[i] == 0: continue',
'    if dir_[i-1] == -1 and dir_[i] ==  1 and rv[i] < 65: mrk(i, "buy",  "ST↑")',
'    if dir_[i-1] ==  1 and dir_[i] == -1 and rv[i] > 35: mrk(i, "sell", "ST↓")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_linear_reg: _PY_H + [
'# [Python] 50-bar Linear Regression Channel ±2σ',
'P = 50',
'reg = [None]*n; u2 = [None]*n; l2 = [None]*n',
'for i in range(P-1, n):',
'    sx=sy=sxy=sx2=0',
'    for j in range(P):',
'        x = j; y = cl[i-P+1+j]',
...
'    icp = (sy - slp*sx) / P',
'    reg[i] = icp + slp*(P-1)',
'    se = math.sqrt(sum((cl[i-P+1+j]-(icp+slp*j))**2 for j in range(P))/P)',
'    u2[i] = reg[i] + 2*se; l2[i] = reg[i] - 2*se',
'plo("LinReg", reg, "#94a3b8")',
'plo("LR +2σ", u2, "#6366f1")',
'plo("LR -2σ", l2, "#6366f1")',
'for i in range(1, n):',
'    if l2[i] is None or l2[i-1] is None: continue',
'    if cl[i-1] <= l2[i-1] and cl[i] > l2[i]: mrk(i, "buy",  "LR")',
'    if cl[i-1] >= u2[i-1] and cl[i] < u2[i]: mrk(i, "sell", "LR")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_rsi_divergence: _PY_H + [
'# [Python] RSI Divergence Detector',
'rv = rsi(cl, 14)',
'plo("RSI",    rv,         "#818cf8")',
'plo("EMA 20", ema(cl,20), "#94a3b8")',
'LB = 5',
'for i in range(LB+1, n-1):',
'    if rv[i] is None or rv[i-LB] is None: continue',
'    pLo = all(cl[i] < cl[i-k] for k in range(1, LB+1))',
'    rLo = all(rv[i] < rv[i-k] for k in range(1, LB+1) if rv[i-k] is not None)',
'    pHi = all(cl[i] > cl[i-k] for k in range(1, LB+1))',
'    rHi = all(rv[i] > rv[i-k] for k in range(1, LB+1) if rv[i-k] is not None)',
'    if pLo and not rLo and rv[i] < 50: mrk(i, "buy",  "Div+")',
'    if pHi and not rHi and rv[i] > 50: mrk(i, "sell", "Div−")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_chandelier_exit: _PY_H + [
'# [Python] Chandelier Exit ATR(22)*3',
'N = 22; mult = 3.0',
'at = atr(N)',
'ls = [None]*n; ss = [None]*n; d = 1',
'for i in range(N, n):',
'    if at[i] is None: continue',
'    hh = max(hi[i-N+1:i+1])',
'    ll = min(lo[i-N+1:i+1])',
'    ls[i] = hh - mult*at[i]',
'    ss[i] = ll + mult*at[i]',
'    if   d ==  1 and cl[i] < ls[i]: mrk(i, "sell", "CE↓"); d = -1',
'    elif d == -1 and cl[i] > ss[i]: mrk(i, "buy",  "CE↑"); d =  1',
'plo("CE Long",  ls, "#10b981")',
'plo("CE Short", ss, "#ef4444")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_heikin_ashi: _PY_H + [
'# [Python] Heikin-Ashi 3-bar streak',
'hac = [0]*n; hao = [0]*n',
'hao[0] = (op[0]+cl[0])/2',
'hac[0] = (op[0]+hi[0]+lo[0]+cl[0])/4',
'for i in range(1, n):',
'    hac[i] = (op[i]+hi[i]+lo[i]+cl[i])/4',
'    hao[i] = (hao[i-1]+hac[i-1])/2',
'plo("HA Close", hac, "#94a3b8")',
'plo("HA EMA 9", ema(hac, 9), "#f59e0b")',
'streak = 0',
'for i in range(1, n):',
'    streak = max(0, streak)+1 if hac[i] >= hao[i] else min(0, streak)-1',
'    if streak ==  3: mrk(i, "buy",  "HA↑")',
'    if streak == -3: mrk(i, "sell", "HA↓")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_volume_rsi: _PY_H + [
'# [Python] Volume-Weighted RSI vs plain RSI',
'P = 14',
'vrsi = [None]*n',
'vag = val = 0.0',
...
'    if i > P:',
'        dv = cl[i]-cl[i-1]',
'        vag = (vag*(P-1) + max(dv,0)*vo[i]) / P',
'        val = (val*(P-1) + max(-dv,0)*vo[i]) / P',
'    vrsi[i] = 100 - 100/(1 + (1e9 if val==0 else vag/val))',
'rv = rsi(cl, P)',
'plo("VRSI",   vrsi,        "#f59e0b")',
'plo("RSI",    rv,           "#818cf8")',
'plo("EMA 20", ema(cl, 20), "#94a3b8")',
'for i in range(1, n):',
'    if vrsi[i] is None or rv[i] is None: continue',
'    if vrsi[i] > rv[i] and vrsi[i] < 45 and vrsi[i-1] <= rv[i-1]: mrk(i, "buy",  "VR↑")',
'    if vrsi[i] < rv[i] and vrsi[i] > 55 and vrsi[i-1] >= rv[i-1]: mrk(i, "sell", "VR↓")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_multi_factor: _PY_H + [
'# [Python] Multi-Factor Composite Signal',
'def zs_val(i, P):',
'    if i < P-1: return 0',
'    w = cl[i-P+1:i+1]; m = sum(w)/P',
'    sd = math.sqrt(sum((x-m)**2 for x in w)/P)',
'    return (cl[i]-m)/sd if sd else 0',
'def clamp(x, lo_, hi_): return max(lo_, min(hi_, x))',
'e50 = ema(cl, 50)',
'obv = [0]*n',
'for i in range(1, n):',
'    obv[i] = obv[i-1]+(vo[i] if cl[i]>cl[i-1] else -vo[i] if cl[i]<cl[i-1] else 0)',
'oe = ema(obv, 20)',
'plo("EMA 50", e50, "#94a3b8")',
'comp = [None]*n',
'for i in range(50, n):',
'    if not e50[i] or not e50[i-1] or oe[i] is None: continue',
'    f1 = clamp((e50[i]-e50[i-1])/e50[i]*300, -1, 1)',
'    roc = (cl[i]-cl[i-20])/cl[i-20]*100 if i>=20 else 0',
'    f2 = clamp(roc/5, -1, 1)',
'    f3 = clamp(-zs_val(i, 30)/2, -1, 1)',
'    f4 = 1 if obv[i] > oe[i] else -1',
'    comp[i] = f1*0.35 + f2*0.30 + f3*0.15 + f4*0.20',
'viz = [e50[i]*(1+comp[i]*0.015) if e50[i] and comp[i] is not None else None for i in range(n)]',
'plo("Factor", viz, "#a855f7")',
'THR = 0.55',
'for i in range(1, n):',
'    if comp[i] is None or comp[i-1] is None: continue',
'    if comp[i-1] <= THR  and comp[i] >  THR: mrk(i, "buy",  "F+")',
'    if comp[i-1] >= -THR and comp[i] < -THR: mrk(i, "sell", "F−")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_williams_fractal: _PY_H + [
'# [Python] Williams Fractal + Alligator',
'def smma(a, p):',
'    r = [None]*n',
'    r[p-1] = sum(a[:p])/p',
'    for i in range(p, n): r[i] = (r[i-1]*(p-1)+a[i])/p',
'    return r',
'med = [(hi[i]+lo[i])/2 for i in range(n)]',
'jaw   = smma(med, 13)',
'teeth = smma(med, 8)',
'lips  = smma(med, 5)',
'plo("Jaw (13)",  jaw,   "#3b82f6")',
'plo("Teeth (8)", teeth, "#ef4444")',
'plo("Lips (5)",  lips,  "#10b981")',
'for i in range(2, n-2):',
'    if teeth[i] is None: continue',
'    fhi = hi[i]>hi[i-1] and hi[i]>hi[i-2] and hi[i]>hi[i+1] and hi[i]>hi[i+2]',
'    flo = lo[i]<lo[i-1] and lo[i]<lo[i-2] and lo[i]<lo[i+1] and lo[i]<lo[i+2]',
'    awU = lips[i] > teeth[i] and teeth[i] > jaw[i]',
'    awD = lips[i] < teeth[i] and teeth[i] < jaw[i]',
'    if flo and awU: mrk(i, "buy",  "▲F")',
'    if fhi and awD: mrk(i, "sell", "▼F")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_elder_impulse: _PY_H + [
'# [Python] Elder Impulse System',
'def macd_hist(a, f, s, sig):',
'    mf = ema(a, f); ms = ema(a, s)',
'    ml = [mf[i]-ms[i] if mf[i] and ms[i] else None for i in range(n)]',
'    sl = ema(ml, sig)',
'    return [ml[i]-sl[i] if ml[i] is not None and sl[i] is not None else None for i in range(n)]',
'e13  = ema(cl, 13)',
'hist = macd_hist(cl, 12, 26, 9)',
'plo("EMA 13", e13, "#94a3b8")',
'for i in range(2, n):',
'    if not e13[i] or not e13[i-1] or hist[i] is None or hist[i-1] is None: continue',
'    nowG = e13[i]>e13[i-1] and hist[i]>hist[i-1]',
'    nowR = e13[i]<e13[i-1] and hist[i]<hist[i-1]',
'    prvG = e13[i-1]>e13[i-2] and hist[i-1]>hist[i-2]',
'    prvR = e13[i-1]<e13[i-2] and hist[i-1]<hist[i-2]',
'    if not prvG and nowG: mrk(i, "buy",  "⚡")',
'    if not prvR and nowR: mrk(i, "sell", "⚡")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_inside_bar: _PY_H + [
'# [Python] Inside Bar Breakout (EMA50 trend filter)',
'e50 = ema(cl, 50)',
'ibH = [None]*n; ibL = [None]*n; ibs = []',
'for i in range(1, n):',
'    if hi[i] <= hi[i-1] and lo[i] >= lo[i-1]:',
'        ibs.append(i)',
'        ibH[i] = hi[i-1]',
'        ibL[i] = lo[i-1]',
'plo("IB High", ibH, "#f59e0b")',
'plo("IB Low",  ibL, "#6366f1")',
'plo("EMA 50",  e50, "#94a3b8")',
'for idx in ibs:',
'    nxt = idx+1',
'    if nxt >= n or e50[nxt] is None: continue',
'    mH = hi[idx-1]; mL = lo[idx-1]',
'    if cl[nxt] > mH and cl[nxt] > e50[nxt]: mrk(nxt, "buy",  "IB↑")',
'    if cl[nxt] < mL and cl[nxt] < e50[nxt]: mrk(nxt, "sell", "IB↓")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),

    py_mean_rev_backtest: _PY_H + [
'# [Python] Z-Score Mean Reversion Backtest + Equity Curve',
'PER = 30; zEn = -1.8; zEx = -0.5',
'ma = sma(cl, PER)',
'plo("SMA 30", ma, "#94a3b8")',
'def rolz(i):',
'    if i < PER-1 or not ma[i]: return 0',
'    w = cl[i-PER+1:i+1]; m = ma[i]',
'    sd = math.sqrt(sum((x-m)**2 for x in w)/PER)',
'    return (cl[i]-m)/sd if sd else 0',
'equity = [None]*n; pnl = 0.0; epx = None',
'for i in range(PER, n):',
'    z = rolz(i)',
'    if epx is None and z < zEn:',
'        epx = cl[i]; mrk(i, "buy", "z↓")',
'    elif epx is not None and z > zEx:',
'        pnl += (cl[i]-epx)/epx; epx = None; mrk(i, "sell", "z↑")',
'    equity[i] = ma[i]*(1+pnl*0.3) if ma[i] else None',
'plo("Equity", equity, "#10b981")',
'zHi = [ma[i]+abs(zEn)*math.sqrt(sum((cl[j]-ma[i])**2 for j in range(i-PER+1,i+1))/PER)',
'       if ma[i] else None for i in range(n)]',
'zLo = [ma[i]+zEn*math.sqrt(sum((cl[j]-ma[i])**2 for j in range(i-PER+1,i+1))/PER)',
'       if ma[i] else None for i in range(n)]',
'plo("z +1.8σ", zHi, "#a855f7")',
'plo("z −1.8σ", zLo, "#a855f7")',
'print(json.dumps({"plots": plots, "markers": markers}))',
    ].join('\n'),
};
